...
If you do not want the connector to capture _truncate_ events, use the xref:postgresql-property-skipped-operations[`skipped.operations`] option to filter them out.
...
* xref:postgresql-time-precision-mode-isostring[`time.precision.mode=isostring`]
* xref:postgresql-time-precision-mode-microseconds[`time.precision.mode=microseconds`]
* xref:postgresql-time-precision-mode-nanoseconds[`time.precision.mode=nanoseconds`]
...
[[postgresql-time-precision-mode-isostring]]
.`time.precision.mode=isostring`
Set the `time.precision.mode` property to `isostring` to configure the connector to map temporal values as ISO-8601 formatted string at UTC time zone.
...
[[postgresql-time-precision-mode-microseconds]]
.`time.precision.mode=microseconds`
Set the `time.precision.mode` property to `microseconds` to configure the connector to specify temporal values with microsecond precision.
...
[[postgresql-time-precision-mode-nanoseconds]]
.`time.precision.mode=nanoseconds`
Set the `time.precision.mode` property to `nanoseconds` to configure the connector to specify temporal values with nanosecond precision.
...
[id="postgresql-tsvector-types"]
=== TSVECTOR types

The PostgreSQL connector supports the native `TSVECTOR` data type without requiring any extensions.

.Mappings of PostgreSQL `TSVECTOR` data type
[cols="25%a,20%a,55%a",options="header"]
|===
|PostgreSQL data type
|Literal type (schema type)
...
Contains the string representation of a `TSVECTOR` value in normalized lexeme format.

Example: `'direct':6 'insert':8 'test':4`

The `TSVECTOR` type is a native PostgreSQL data type (OID 3614) that is used for full-text search operations.
The connector converts `TSVECTOR` values to their string representation, preserving the lexemes and their positions.
...
* xref:debezium-postgresql-connector-specify-the-plugin-name[]
* xref:debezium-postgresql-connector-create-a-replication-user[]
* xref:postgresql-server-configuration[]
* xref:enabling-postgresql-failover-slots[]
...
* xref:supported-postgresql-topologies[]
* xref:configuring-postgresql-to-manage-debezium-wal-disk-space-consumption[]
* xref:postgresql-deploying-multiple-connectors[]
...
To use {prodname} with link:https://cloud.google.com/sql/postgresql/[Cloud SQL for PostgreSQL], you must configure the database to use the `pgoutput` logical decoding plug-in.
...
It is possible to use {prodname} with link:https://crunchybridge.com/[CrunchyBridge]; logical replication is already turned on.
The `pgoutput` plugin is available.
You will have to create a replication user and provide correct privileges.

[IMPORTANT]
====
While using the `pgoutput` plug-in, it is recommended that you configure `filtered` as the xref:postgresql-publication-autocreate-mode[`publication.autocreate.mode`].
If you use `all_tables`, which is the default value for `publication.autocreate.mode`, and the publication is not found, the connector tries to create one by using `CREATE PUBLICATION <publication_name> FOR ALL TABLES;`, but this fails due to lack of permissions.
====
...
[WARNING]
====
The {prodname} logical decoding plug-ins have been installed and tested on only Linux machines.
For Windows and other operating systems, different installation steps might be required.
====
...
// Type: procedure
[id="debezium-postgresql-connector-specify-the-plugin-name"]
=== Specify the {prodname} `plugin.name`

ifdef::community[]
In environments that require the connector to use the `pgoutput` logical decoding plug-in, such as Google Cloud SQL for PostgreSQL or PostgreSQL on Amazon RDS, you must explicitly set the value of the connector's `plugin.name` property to `pgoutput`.
endif::community[]
ifdef::product[]
For the connector to capture change events from a PostgreSQL database, you must configure it to use the `pgpoutput` logical decoding plug-in.
Setting this property is required, even though the connector does not support use of alternate decoding plug-ins.
endif::product[]

.Procedure

* In the {prodname} connector configuration, set the value of the `plugin.name` property to `pgoutput`, as in the following example:
+
ifdef::community[]
[source,json]
----
...
//Type: procedure
[id="debezium-postgresql-connector-create-a-replication-user"]
...
+
Create a user with the `REPLICATION` attribute::
...
Set the `REPLICATION` attribute for an existing user::
Log in as the `postgres` user, or as a member of the `cloudsqlsuperuser` user group, and run the following command:
+
----
...
// Type: assembly
// Title: Configure the PostgreSQL server for {prodname}
...
----
The preceding statement instructs the PostgreSQL server to load the `decoderbufs` logical decoding plug-ins at startup (the name of the plug-in is set in the link:https://github.com/debezium/postgres-decoderbufs/blob/v{debezium-version}/Makefile[`Protobuf`] make file).
...
wal_level = logical
----
The preceding statement instructs the PostgreSQL server to use logical decoding with the write-ahead log.
endif::community[]

// Type: concept
// Title: PostgreSQL replication parameters and performance considerations
[id="debezium-postgresql-connector-replication-parameters-and-performance-considerations"]
==== Replication parameters and performance considerations

Depending on your requirements, you can set other PostgreSQL streaming replication parameters to customize performance in a {prodname} environment.
For example, you can increase the number of connectors that can access the sending server concurrently by setting the `max_wal_senders` and `max_replication_slots` parameters.
Or you can set `wal_keep_size` to limit the maximum WAL size of a replication slot.

{prodname} uses PostgreSQL's logical decoding, which uses replication slots.
Logical decoding extracts data changes from the PostgreSQL write-ahead log (WAL) and converts them to a format that {prodname} can consume.
Replication slots are guaranteed to retain all WAL segments required for {prodname} even during {prodname} outages.
For this reason, it is important to closely monitor replication slots to avoid excess disk consumption and conditions such as catalog bloat that can result if a replication slot is unused for too long.

To minimize the latency of change events when the PostgreSQL `synchronous_commit` parameter is set to values other than `on`, set the `wal_writer_delay` for the cluster to a value such as 10 milliseconds
If the property does not specify an explicit value, {prodname} applies the default value of 200 milliseconds, which increase latency.
...
* link:https://www.postgresql.org/docs/current/runtime-config-replication.html#RUNTIME-CONFIG-REPLICATION-SENDER[Configuring PostgreSQL streaming replication (PostgreSQL documentation)^].
* link:https://www.postgresql.org/docs/current/warm-standby.html#STREAMING-REPLICATION-SLOTS[PostgreSQL streaming replication (PostgreSQL documentation)^].


// Type: procedure
// ModuleID: debezium-postgresql-connector-enabling-failover-slots
[id="enabling-postgresql-failover-slots"]
==== Enable failover slots for the PostgreSQL connector

When {prodname} connects to a PostgreSQL primary server that runs PostgreSQL 17 or later, you can configure the connector to create replication slots that support failover.
Failover-enabled replication slots enable {prodname} to continue reading changes from a new primary server after a failover event, without losing events.

.Prerequisites

* {prodname} connects to a primary server that runs PostgreSQL 17 or later.
* You have administrative access to configure PostgreSQL server parameters.
* You have configured the {prodname} connector properties.

.Procedure

. In the {prodname} connector configuration, set the xref:postgresql-property-slot-failover[`slot.failover`] property to `true`.
+
...
. Verify that the failover slot is created and synchronized.
+
You can query the `pg_replication_slots` view to confirm that the slot exists and that the `failover` column is set to `true`:
+
[source,sql]
----
...
* link:https://www.postgresql.org/docs/current/logical-replication-failover.html[PostgreSQL failover slot behavior (PostgreSQL documentation)^].
* link:https://www.postgresql.org/docs/current/warm-standby.html#STREAMING-REPLICATION-SLOTS[PostgreSQL streaming replication slots (PostgreSQL documentation)^].
* xref:postgresql-property-slot-failover[`slot.failover` connector property]
* xref:postgresql-cluster-failures-recovering-from-failures-pg17-cluster[Recovering from failures in a PostgreSQL 17 cluster].


// Type: procedure
// ModuleID: setting-up-postgresql-permissions-required-by-debezium-connectors
// Title: Set up PostgreSQL permissions for the {prodname} connector
...
// Title: Set privileges to enable {prodname} to create PostgreSQL publications
...
// Title: Configure PostgreSQL to permit replication with the {prodname} host
...
[NOTE]
====
For more information about network masks (netmasks), see link:https://www.postgresql.org/docs/current/static/datatype-net-types.html[Network address types (PostgreSQL documentation)].
====
...
// Type: concept
// Title: Replication slot configuration when multiple connectors capture from a database server
[[postgresql-deploying-multiple-connectors]]
=== Setting up multiple connectors for the same database server

When you deploy multiple {prodname} connectors to capture changes from the same PostgreSQL database server, each connector requires a unique replication slot and publication.
This configuration prevents data loss and ensures that each connector independently tracks its position in the database transaction log.

{prodname} uses replication slots to stream changes from a database.
Replication slots use a log sequence number (LSN) as a persistent pointer to track the last position in the write-ahead Log (WAL) that the connector has processed.
This tracking mechanism enables PostgreSQL to retain WAL segments until {prodname} processes them.

Because each replication slot maintains a single LSN pointer, it can track only one consumer's position in the WAL.
When multiple connectors attempt to share a replication slot, they compete for the same position marker, resulting in unpredictable behavior and potential data loss.
...
When multiple connectors share a slot, PostgreSQL delivers each change event to only one of the competing consumers.
The other connectors never receive those events, resulting in incomplete data capture.
PostgreSQL provides no warnings when this misconfiguration occurs, making the data loss silent and difficult to detect.
====

In addition to replication slots, when the {prodname} connector uses the `pgoutput` plugin, it uses publications to stream events.
Publications exist at the database level and define the set of tables from which the connector captures changes.
Like replication slots, publications should be unique to each connector to ensure proper isolation and independent configuration.

While multiple connectors can technically share a publication if they capture changes from the same set of tables, this configuration still requires unique replication slots for each connector.
Sharing publications is appropriate only when you intentionally want multiple connectors to monitor identical table sets, such as when distributing processing load across multiple pipelines.

. Additional resources
* https://www.postgresql.org/docs/18/logicaldecoding-explanation.html#LOGICALDECODING-REPLICATION-SLOTS[PostgreSQL replication slot details and resource implications. (PostgreSQL documentation)^].
* xref:postgresql-publication-autocreate-mode[Description of the `publication.autocreate.mode` property for specifying how {prodname} creates publication to control table inclusion].
* xref:postgresql-property-slot-name[Description of the `slot.name` property for specifying a unique replication slot name for each connector].
* xref:postgresql-property-publication-name[Description of the `publication.name` property for specifying a unique publication name for each connector].

// Type: procedure
// ModuleID: upgrading-postgresql-databases-that-debezium-captures-from
// Title: Upgrade PostgreSQL databases that {prodname} captures from
...
If you are working with immutable containers, see link:https://quay.io/organization/debezium[{prodname}'s container images] for Kafka, PostgreSQL and Kafka Connect with the PostgreSQL connector already installed and ready to run.
...
You must explicitly set `plugin.name` to `pgoutput`.
endif::product[]
...
|[[postgresql-property-offset-mismatch-strategy]]<<postgresql-property-offset-mismatch-strategy, `+offset.mismatch.strategy+`>>
|`no_validation`
|Specifies how the connector handles mismatches between the stored offset LSN and the replication slot's confirmed flush LSN when the connector starts.
+
[IMPORTANT]
====
The `offset.mismatch.strategy` property is a Technology Preview feature.
...
====
This property replaces the deprecated `internal.slot.seek.to.known.offset.on.start` boolean property.

A mismatch can occur in several scenarios:

* The replication slot has been manually advanced using `pg_replication_slot_advance()`.
* When using xref:postgresql-property-lsn-flush-mode[`lsn.flush.mode=connector_and_driver`], the JDBC driver's keep-alive mechanism has flushed LSNs for unmonitored WAL activity, advancing the slot ahead of the stored offset.
* The replication slot has been recreated after being dropped.
* Kafka Connect offset storage has been corrupted or reset.

Set one of the following options:

`no_validation`::
The connector attempts to stream from the stored offset without validating the replication slot state.
If the slot is ahead of the offset, PostgreSQL returns an error because the requested LSN is no longer available in the WAL.

`trust_offset`::
The connector validates that the stored offset is not behind the replication slot's confirmed flush LSN.
If the offset is behind the slot, the connector fails with an error indicating potential data loss.
If the offset is ahead of or equal to the slot, the connector advances the slot to the offset position when possible.
Use this strategy when you want to detect and be alerted to unexpected slot state changes that could indicate data loss.

`trust_slot`::
The connector treats the PostgreSQL replication slot as the authoritative source of truth.
If the stored offset is behind the slot's confirmed flush LSN, the connector automatically advances the offset to match the slot position.
+
NOTE: This strategy skips replaying events between the stored offset and the slot position.
Use this strategy when you trust that the replication slot reliably represents all logical replication events you need to capture.
This is appropriate when using xref:postgresql-property-lsn-flush-mode[`lsn.flush.mode=connector_and_driver`], which requires trusting the slot position.
Double-check that your environment ensures replication slot durability during primary-replica failover.

`trust_greater_lsn`::
The connector synchronizes to the maximum LSN between the stored offset and the slot's confirmed flush LSN.
If the offset is behind the slot, the connector advances the offset to the slot position.
If the offset is ahead of the slot, the connector advances the slot to the offset position when possible.
This provides bidirectional synchronization and is useful when using xref:postgresql-property-lsn-flush-mode[`lsn.flush.mode=connector_and_driver`].
The same slot reliability requirements as `trust_slot` apply.
+
NOTE: When the slot is ahead of the offset, this strategy skips replaying events between the stored offset and the slot position.

For backward compatibility, the deprecated `internal.slot.seek.to.known.offset.on.start` property is automatically mapped:

* `internal.slot.seek.to.known.offset.on.start=false` maps to `offset.mismatch.strategy=no_validation`
* `internal.slot.seek.to.known.offset.on.start=true` maps to `offset.mismatch.strategy=trust_offset`
...
ifdef::community[]
|[[postgresql-property-database-connection-class-factory]]<<postgresql-property-database-connection-class-factory, `+database.connection.factory.class+`>>
|No default
|The name of the ConnectionFactory.
One use of this property is to specify the interface that encapsulates credentials and other connection details when you use AWS IAM Authentication to establish a JDBC connection to a PostgreSQL database on Amazon RDS.
For more information, see xref:postgresql-on-amazon-rds[PostgreSQL on Amazon RDS]).
endif::community[]
...
|[[postgresql-property-database-sslfactory]]<<postgresql-property-database-sslfactory, `+database.sslfactory+`>>
|No default
|A name of the class that creates SSL Sockets.
Use `org.postgresql.ssl.NonValidatingFactory` to disable SSL validation in development environments.
...
[IMPORTANT]
====
If the expressions that you specify for this property match columns that are not part of the table's primary key,
set the `REPLICA IDENTITY` of the table to `FULL`.
If you set `REPLICA IDENTITY` to another value, such as `DEFAULT`, after delete operations, the connector fails to generate tombstone events with the expected `null` values.
====
...
[NOTE]
To create publications, the connector must access PostgreSQL through a database account that has specific permissions.
...
|No default
|A list of optional key-value pairs, separated by semicolons, that represent parameters that the connector passes to the configured PostgreSQL logical decoding plug-in when the replication stream starts.
The connector passes the specified parameters as-is to the plug-in.
Valid parameters depend on the configured logical decoding plug-in.
Parameters that are not relevant to the configured plug-in are ignored.

ifdef::community[]
`decoderbufs` plugin::
If the connector uses the `decoderbufs` plug-in, you can set the `debug-mode` parameter to instruct a replication slot to log logical decoding debug messages.
When {prodname} sends the SQL command to create a replication slot, it passes the `debug-mode` parameter.
For example:

 debug-mode=true

NOTE: To enable viewing of debug messages, you must set the PostgreSQL logging level (https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-MIN-MESSAGES[`log_min_messsages`]) to DEBUG1 or higher.
endif::community[]

`pgoutput` plugin::
For connectors that use the `pgoutput` plug-in, add the `origin` parameter to the `slot.stream.params` property to specify whether a subscription requests the publisher to send all changes, or only local changes.
+
For example to request the publisher to send all changes, that is, changes that originate locally on the publisher, and changes that were replicated to the publisher from another source, set the following value:
...
|`false`
|Specifies how the connector captures and emits events for changes that it captures from partitioned tables.
This setting applies only if the xref:postgresql-publication-autocreate-mode[`publication.autocreate.mode`] property is set to `all_tables` or `filtered`, and {prodname} creates the publication for the captured tables.
...
|`true`
|**Deprecated.** Use xref:postgresql-property-lsn-flush-mode[`lsn.flush.mode`] instead.

For backward compatibility, this property is automatically mapped to `lsn.flush.mode`:

* `flush.lsn.source=true` maps to `lsn.flush.mode=connector`
* `flush.lsn.source=false` maps to `lsn.flush.mode=manual`

|[[postgresql-property-lsn-flush-mode]]<<postgresql-property-lsn-flush-mode, `+lsn.flush.mode+`>>
|`connector`
|Specifies how the connector manages flushing the LSN (Log Sequence Number) to the PostgreSQL replication slot.
This property replaces the deprecated xref:postgresql-property-flush-lsn-source[`flush.lsn.source`] property.

Set one of the following options:

`manual`::
LSN flushing is managed externally by your application or another mechanism.
The connector does not flush the LSN.
+
NOTE: If you do not configure an external mechanism to flush the LSN, WAL files accumulate on the database server, potentially causing storage exhaustion and performance degradation.

`connector`::
{prodname} flushes the LSN after processing each logical replication change event.
The PostgreSQL JDBC driver's keep-alive thread does not flush LSNs.
This is the default mode.

`connector_and_driver`::
Both {prodname} and the PostgreSQL JDBC driver's keep-alive thread can flush LSNs.
When the connector has no pending LSN to flush, the JDBC driver's keep-alive mechanism can flush the server-reported keep-alive LSN.
This LSN reflects all WAL activity, including unmonitored activity such as `CHECKPOINT`, `VACUUM`, or `pg_switch_wal()` that doesn't produce logical replication change events.
Use this mode to prevent WAL growth on low-activity databases where monitored tables have infrequent changes.
+
...
|[[postgresql-property-guardrail-collections-max]]<<postgresql-property-guardrail-collections-max, `+guardrail.collections.max+`>>
|`0`
...
|[[postgresql-property-guardrail-collections-limit-action]]<<postgresql-property-guardrail-collections-limit-action, `+guardrail.collections.limit.action+`>>
|`warn`
...
|[[postgresql-property-extended-headers-enabled]]<<postgresql-property-extended-headers-enabled, `extended.headers.enabled`>>
|true
...
[id="debezium-postgresql-connector-pass-through-properties"]
...
The {prodname} PostgreSQL connector provides two types of metrics that are in addition to the built-in support for JMX metrics that Kafka and Kafka Connect provide.
