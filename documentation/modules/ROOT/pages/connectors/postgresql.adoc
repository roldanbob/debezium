...
When {prodname} connects to a PostgreSQL primary (that is, not a read replica) on PostgreSQL 17 or later, you can configure the connector to that it creates replication slots that are enabled for https://www.postgresql.org/docs/current/logical-replication-failover.html[failover].
Failover slots permit {prodname} to continue to read changes from a replica that is promoted to primary in case of a failure, without missing any events.
This requires the state of the slot to be synchronized from primary to replica server.
For more information about the behavior of failover slots, see the PostgreSQL documentation.

If you are working with a `synchronous_commit` setting other than `on`, set `wal_writer_delay` to a value such as 10 milliseconds to achieve a low latency of change events.
IF you do not set an explicit value, {prodname} applies the default value, which adds a latency of about 200 milliseconds.
