...
// Type: concept
[id="debezium-oracle-chunk-based-parallel-snapshots"]
=== Chunk-based parallel snapshots

include::{partialsdir}/modules/all-connectors/con-connector-chunk-based-parallel-snapshots.adoc[leveloffset=+3]
...
The following list describes select fields in the preceding schema change topic message example:

`ts_ms`, `ts_us`, `ts_ns`::
Optional fields that display the time at which the connector processed the event in millisecond, microsecond, and nanosecond formats.
The time is based on the system clock that runs the JVM that runs the Kafka Connect task.
+
...
These fields identify the database and the schema that contains the change.
...
The `redo_log_catalog` mining strategy provides reliable schema change tracking at the cost of higher redo log volume and archive log consumption.
...
==== Important considerations when the connector uses a downstream mining database

To use the connector with a downstream mining database, you must first register transaction logs with that database.

When Oracle Remove File Server (RFS) replicates changes, the replication should happen automatically when using a real-time downstream mining configuration. 
{prodname} does not perform any log registration.

If a log fails to register and a gap in log sequences occurs, the {prodname} connector pauses capture. A database administrator must review the registered logs and register any missing logs to close the gap. 
Typically, this should be necessary only during the initial set up and synchronization with the primary.
...
The heap buffer stores in-progress transaction data in the JVM process memory, making it suitable for environments where transactions are short-lived and transaction volume is moderate.
...
You must configure caches for the following identifiers: `transactions`, `events`, `processed-transactions`, and `schema-changes`.
...
----
The following example shows an Ehcache configuration that includes both a global setting and specific settings for the `transactions`, `processedtransactions`, `schemachanges`, and `events` caches.
...
// Type: concept
// ModuleID: how-debezium-captures-large-object-data
// Title: How {prodname} captures large object data
[[capturing-large-object-data]]
=== Capturing large object data

Capturing Oracle LOB data during streaming requires special handling.
Configure the connector to capture LOB values accurately while balancing mining overhead and performance.

To accurately capture LOB column values during streaming, set the `lob.enabled` connector configuration property to `true`.
With this property enabled, the connector can capture the following data types:

`CLOB`:: Character large objects.
`NCLOB`:: National character set large objects.
`BLOB`:: Binary large objects.
`XMLTYPE`:: XML data stored as a LOB.
`JSON`:: JSON data (Oracle 21c and later).
`Extended strings`:: `VARCHAR2`, `NVARCHAR2`, and `RAW` columns that exceed the legacy maximum lengths when the database parameter `max_string_size` is set to `EXTENDED`.
For more information about how the connector supports extended strings, see xref:support-for-extended-max-string-size[Extended max string size].

When `lob.enabled` is set to `false` (the default), snapshot events include the current LOB column values, but streamed change events represent LOB columns with the xref:oracle-property-unavailable-value-placeholder[`unavailable.value.placeholder`] constant.
Disabling mining of LOB values reduces the volume of data that LogMiner processes by excluding LOB-specific redo log entries.

When `lob.enabled` is `true`, the connector mines additional redo log operation types that Oracle uses to represent LOB writes.
During streaming, Oracle includes LOB column values in redo data only when a SQL DML statement explicitly sets or changes an LOB column value.
If a DML statement does not modify an LOB column, the change event contains the configured unavailable value placeholder rather than the column's current value.

// Type: concept
// Title: How Oracle processes INTERNAL events and savepoint rollbacks
// ModuleID: how-oracle-processes-internal-events-and-savepoint-rollbacks
[id="oracle-internal-events-and-rollback-to-savepoint-handling"]
==== INTERNAL events and ROLLBACK TO SAVEPOINT handling

Oracle records INTERNAL events in the redo log for out-of-line LOB operations.
These events serve two purposes: they associate `ROW_ID` values with LOB data, and they mark the boundary between successive DML statements.
The connector uses these boundaries to identify which events belong to a rolled-back statement during `ROLLBACK TO SAVEPOINT` processing.
Set the `log.mining.include.internal.events` property to specify whether the connector captures `INTERNAL` events.
The connector requires these events to accurately process `ROLLBACK TO SAVEPOINT` operations in transactions that modify LOB columns.

When a table stores LOB data out-of-line, these events provide the connector with the row identifier that it needs to correctly correlate LOB changes with their parent DML operation.
This correlation becomes critical when a transaction uses `ROLLBACK TO SAVEPOINT` to partially undo changes.
In the absence of the `ROW_ID` information that an `INTERNAL` event provides, the connector cannot reliably determine which row a rolled-back LOB operation affects.
In such scenarios, the connector might emit change events that contain stale LOB field values.

The `log.mining.include.internal.events` property controls whether the connector includes Oracle `INTERNAL` events in LogMiner queries.
The `log.mining.include.internal.events` property accepts the following values:

`false`:: (Default) The connector excludes `INTERNAL` events from LogMiner queries.
This setting reduces the volume of mined redo log data.
However, after a partial rollback of a transaction that modifies out-of-line LOB columns, the connector might emit change events that contain stale values for affected LOB fields.

`true`:: The connector includes `INTERNAL` events in LogMiner queries.
This setting increases the volume of mined redo log data, but enables the connector to accurately process `ROLLBACK TO SAVEPOINT` operations for tables with out-of-line LOB columns.

[NOTE]
====
The `log.mining.include.internal.events` property has no effect unless `lob.enabled` is also set to `true`.
====

[TIP]
====
If your application does not use `ROLLBACK TO SAVEPOINT` in transactions that modify tables with LOB columns, you can safely leave `log.mining.include.internal.events` set to `false`.
Enable it only when you need accurate partial rollback handling for tables that store LOB data out-of-line.
====

As an alternative to enabling LOB mining and capturing `INTERNAL` events, you can set `lob.enabled` to `false` and configure the xref:post-processors/reselect-columns.adoc[Reselect columns post processor] to re-query LOB column values directly from the database.

This approach avoids the overhead of mining LOB-specific redo log entries and `INTERNAL` events while still ensuring that change events contain correct LOB values, because the post processor retrieves the values directly from the source table.
...
----
The preceding change event key describes a row in the `inventory.customers` table that the connector named `server1`) that has primary key `id` with a value of `1004`.
...
`schema.fields.type`::
Specifies that the data type of the `ID`field is `int32`.
...
The value of the `payload` field is a structure (that is, a JSON object), that contains a single `ID` field with the value `1004`.
...
The following list describes select fields in the preceding _truncate_ event value example:

`source`::
Mandatory field that describes the source metadata for the event.
In a _truncate_ event value, the `source` field structure is the same as for _create_, _update_, and _delete_ events for the same table, and provides this metadata:
+
* {prodname} version
...
The {prodname} Oracle connector supports the `XMLTYPE` data type and maps it as a string value.
...
Before you deploy the {prodname} Oracle connector, you must configure the Oracle database to run in archive log mode and enable supplemental logging.
...
ifdef::product[]
To deploy a {prodname} Oracle connector, use {StreamsName} to automatically create a Kafka Connect image that includes the connector plug-in.
...
To add the connector to your Kafka Connect image, add the Maven Central location for the driver to `builds.plugins.artifact.url` in the `KafkaConnect` custom resource as shown in xref:using-streams-to-deploy-debezium-oracle-connectors[].
...
`name`::
The name that is assigned to the connector when you register it with a Kafka Connect service.
...
`topic.prefix`::
Topic prefix that identifies and provides a namespace for the Oracle database server from which the connector captures changes.
...
In the preceding example, the `database.hostname` and `database.port` properties define the connection to the database host.
However, in more complex Oracle deployments, or in deployments that use Transparent Network Substrate (TNS) names, you can use an alternative method, which specifies the JDBC URL for the connection.
...
You can configure the {prodname} Oracle connector to use Java key and trust stores when establishing a mutually authenticated TLS connection to the database.
...
You can configure the {prodname} Oracle connector to use Oracle Wallet when establishing a mutually authenticated TLS connection to the database.
...
* xref:debezium-oracle-connector-pass-through-properties[Pass-through Oracle connector configuration properties]
...
The following configuration properties are required to configure the {prodname} Oracle connector unless a default value is specified.
...
If a value greater than 5000 milliseconds (5 seconds) is provided, the effective wait time will be capped at 5000 milliseconds.

|[[oracle-property-poll-dispatch-interval-ms]]<<oracle-property-poll-dispatch-interval-ms, `+poll.dispatch.interval.ms+`>>
|`0`
|An integer value that specifies the number of milliseconds the connector should accumulate events in the queue before each queue consumption.
...
|[[oracle-property-log-mining-include-internal-events]]<<oracle-property-log-mining-include-internal-events, `+log.mining.include.internal.events+`>>
|`false`
|Specifies whether the connector captures `INTERNAL` events.
Set one of the following values:
`false`:: (default) The connector does not capture `INTERNAL` events.
Set this option to reduce the volume of redo log data that the connector processes.
After a partial rollback of a transaction that modifies externally stored LOB data, the change events that the connector emits might contain stale values for the affected LOB fields.

`true`:: The connector captures `INTERNAL` events.
Set this option to enable the connector to accurately handle `ROLLBACK TO SAVEPOINT` operations for tables with out-of-line LOB columns.
This option can result in an increased volume of redo log data.
For more information, see xref:capturing-large-object-data[Capturing large object data].
...
The value must be a positive integer greater than or equal to `1`.
When set to a value greater than `1`, the connector performs a parallel snapshot by dividing each table into chunks based on primary key ranges and processing the chunks concurrently across all available threads.
...
|[[oracle-property-snapshot-max-threads-multiplier]]<<oracle-property-snapshot-max-threads-multiplier, `+snapshot.max.threads.multiplier+`>>
|`1`
|A global multiplier that controls the number of chunks created per table during a parallel snapshot.
By default, {prodname} creates one chunk per thread.
A higher value causes the connector to create more, smaller chunks per table when running a snapshot.
Smaller chunks keep threads more evenly loaded and reduce the amount of work that must be repeated if a snapshot is interrupted.
For example, with 4 threads and a multiplier of `2`, {prodname} creates 8 chunks instead of 4.
...
|[[oracle-property-legacy-snapshot-max-threads]]<<oracle-property-legacy-snapshot-max-threads, `+legacy.snapshot.max.threads+`>>
|`false`
|The default value (`false`) enables the connector to accelerate initial snapshots by dividing tables into chunks and using separate threads to process each chunk concurrently.
To revert to the legacy parallel snapshot behavior of processing one table per thread, set the value to `true`.

[NOTE]
====
When the legacy behavior is enabled, threads that complete their table snapshot remain idle while waiting for other threads to complete.
...
An exception can then result, even if the snapshot captures all data successfully.
If you experience this problem, set `snapshot.max.threads` to `1` and retry the snapshot.
====
ifdef::community[]

NOTE: The property `internal.legacy.snapshot.max.threads` is a deprecated alias for `legacy.snapshot.max.threads` and should not be used in new configurations.
endif::community[]
ifdef::product[]
...
By default, the {prodname} Oracle connector uses the native Oracle LogMiner to ingest changes.
As an alternative to LogMiner, you can configure the connector to use OpenLogReplicator, a free, open-source, third-party application to read changes directly from the Oracle redo and archive logs with low database impact.
...
* OpenLogReplicator is installed with direct access to the archive and redo log files. 
This does not necessarily require installation on the physical database server if the archive and redo logs can be accessed via some shared filesystem.
...
----
{
  ...
  "source": [{
    ...
    "reader": {
      "type": "online",
      "path-mapping": ["/opt/olr/recovery_area", "/opt/olr/recovery_area/ORCLCDB/archivelog"],
      "user": "c##dbzuser",
      "password": "dbz",
      "server": "//<ip>:<port>/ORCLPDB1"
    },
    "format": {
      "type": "json",
      "column": 2,
...
The following list describes select fields in the preceding OpenLogReplicator configuration example:

`name`::
This should match the `openlogreplicator.source` connector configuration.

`reader.path-mapping`::
List of file path pairs `[before1,after1,befor2,after2,...]` where if a log file path matches one of the `beforeX` prefixes, the prefix is replaced with the `afterX` path.
This is useful when OpenLogReplicator is running on a different host than the source database and the path to the redo and archive logs differs between the database and the OpenLogReplicator process.

`reader.user`::
This should match the `database.user` connector property.

`reader.password`::
This should match the `database.password` connector property.

`reader.server`::
This should point to the database host, port, and Oracle SID.

`format`::
Specifies the payload format ingested by the {prodname} Oracle connector.
Use these values as specified, as these are the only format options that we require beyond the defaults.

`flags`::
The sum of the OpenLogReplicator flags that control what the stream includes.
See <<oracle-openlogreplicator-flags>>.

`target.writer.type`::
This must specify `network`, as the {prodname} Oracle connector communicates with OpenLogReplicator via a network connection.

`target.writer.uri`::
Specifies the bind host and port that OpenLogReplicator listens for connections on.
This should be accessible by the {prodname} Oracle connector.
====

[id="oracle-openlogreplicator-flags"]
==== OpenLogReplicator flags

The `flags` property is the sum of a set of bit flags that determine what OpenLogReplicator includes in the stream.
It applies to the source, alongside `format`, rather than within it.

A column that OpenLogReplicator omits because of a missing flag is not reported as an error.
The connector receives no value for it, and the field is `null` in the change event, so a missing flag looks like missing data rather than a configuration problem.

.OpenLogReplicator flags used by {prodname}
[cols="35%a,15%a,50%a"]
|===
|Flag
|Value
...
|`SHOW_DDL`
|`32`
|Includes DDL statements in the stream. Required for the connector to track schema changes.

|`SHOW_HIDDEN_COLUMNS`
|`64`
|Includes columns that Oracle marks as hidden. Required to capture columns that are declared `INVISIBLE`; without it, such a column is silently absent from every change event.

|`SHOW_CHECKPOINT`
|`4096`
|Includes checkpoint markers in the stream. Required for the connector to advance its offsets while no captured table is changing.

|`EXPERIMENTAL_XMLTYPE`
|`65536`
|Includes the values of `XMLTYPE` columns. Required to capture them; without it, such a column is silently absent from every change event. This flag is experimental in OpenLogReplicator.
|===

The value `69728` in the preceding example is the sum of all four.
Omit `65536` if the tables you capture contain no `XMLTYPE` columns, and `64` if they contain no `INVISIBLE` columns.
...
Before you configure the {prodname} Oracle connector to use XStream, you must prepare the Oracle database by enabling archive log mode and supplemental logging.
...
The XStream administrator user requires specific tablespace and privilege grants to authorize the {prodname} Oracle connector to capture change events using XStream.
...
The connector's XStream user requires a dedicated tablespace and a set of privileges that grant access to the database views and objects needed for change data capture.
...
Find answers to common questions about {prodname} Oracle connector version support, LogMiner behavior, offset management, and error recovery.
